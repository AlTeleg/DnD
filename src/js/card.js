...
export default class Card {
    ...
    toColumn() {
        ...
        this.column.insertBefore(div, this.column.lastElementChild.previousElementSibling);
        ...
        const onMouseDown =  (e) => {
            if (e.target.classList.contains('card') || e.target.classList.contains('card-img')) {
        ...
        document.addEventListener('drop', (e) => {
            ...
            if (e.target.classList.contains('card') || e.target.classList.contains('card-title') || e.target.classList.contains('card-icon')) {
                ...
                e.target.closest('.card').insertBefore(cardContentImg, e.target.closest('.card').firstChild.nextElementSibling);
                cardContentImg.style.alignSelf = 'center';
                cardContentImg.classList.add('card-img');
                cardContentImg.style.filter = 'opacity(1)';
                cardContentImg.src = URL.createObjectURL(e.dataTransfer.files && e.dataTransfer.files[0]);
            } 
        })
    }
}
    ...
    window.addEventListener('beforeunload', () => {
        ...
        cardsArray.forEach(card => {
          ...
          if (card.firstElementChild.nextElementSibling.classList.contains('close-cross')) {
            ...
            cardData[card.parentElement.id] = [card.firstChild.value, card.firstElementChild.nextElementSibling.value];
          }
      ...
      document.addEventListener('DOMContentLoaded', () => {
        ...
        if (cardData) {
          Object.keys(cardData).forEach(key => {
            const card = new Card(document.querySelector(`#'${key}'`), cardData[key])
            card.toColumn();
          })
